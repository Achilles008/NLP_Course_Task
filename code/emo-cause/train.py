...
class Instructor:
    ...
    def test(self):
        play_data_loader = BucketIterator(data=self.dataset.test_data[1], batch_size=opt.batch_size, shuffle=False)
        self.model.eval()

        with torch.no_grad():
            for t_batch, t_sample_batched in enumerate(play_data_loader):
                t_inputs = [t_sample_batched[col].to(self.opt.device) for col in self.opt.inputs_cols]
                t_targets = [t_sample_batched[col].to(self.opt.device) for col in self.opt.targets_cols]
                t_outputs = self.model(t_inputs)
